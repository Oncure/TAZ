...
ResonanceTrain = [(idx == np.round(El/(Grouping*dE))).any() for idx in range(N)]
RawDataTrain = Noise(TrueDataTrain)
RawInputTrain = [RawDataTrain[idx-tail:idx+tail+1] for idx in range(tail, Grouping*N+tail, Grouping)]

NeuralNet = MLPClassifier(solver='adam', alpha=1e-2, hidden_layer_sizes=(15,8), activation='relu')
...
# ResonanceTrain = [(idx == np.round(El/dE)).any() for idx in np.round(E[tail:-tail]/dE)] # !!!!!
ResonanceTest = [(idx == np.round(El/(Grouping*dE))).any() for idx in range(N)]
RawDataTest = Noise(TrueDataTest)
RawInputTest = [RawDataTest[idx-tail:idx+tail+1] for idx in range(tail, Grouping*N+tail, Grouping)]
...
Guess = np.array(NeuralNet.predict(RawInputTest))

def ProbCorrPlot(Prob_Guess_All, Answer):
    nBin = round(0.5*np.sqrt(len(Answer)))
...
print(confusion_matrix(ResonanceTest, Guess))
...
plt.hlines([-0.05, 0.05], 0, DE, colors=['r','r'], linewidth=3)
[plt.hlines( 0.05, E[Grouping*idx+tail], E[Grouping*(idx+1)+tail], colors='b', linewidth=3) for idx in range(N) if bool(Guess[idx])]
[plt.hlines(-0.05, E[Grouping*idx+tail], E[Grouping*(idx+1)+tail], colors='g', linewidth=3) for idx in range(N) if ResonanceTest[idx]]
...
Weights = NeuralNet.coefs_
plt.figure()
plt.imshow(Weights[0], 'hot')
plt.tight_layout()
plt.show()
