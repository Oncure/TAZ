...
if __name__ == "__main__":
    ...
    res, SGType = FileReader.readSammyPar('/Users/colefritsch/ENCORE/Python_ENCORE/SAMQUA.PAR')
    ...
    MLS = RMatrix.MeanSpacingEst(res.E, SGType)
    Gnm = RMatrix.MeanNWidthEst(res.Gn, SGType, res.E, l, dfn, A)
    Ggm = RMatrix.MeanGWidthEst(res.Gg, SGType, dfg)

    SGs = SpinGroups.make(l, j)
    MP_True = Resonances.MeanParameters(MLS=MLS, Gnm = Gnm, nDOF = dfn, Ggm = Ggm, gDOF = dfg, A = A, sg = SGs, EB = EB, FreqF=FreqF)
    Res, Types, Missed_Res, Missed_Types = MP_True.sample()

    Freq = MP_True.Freq.reshape(-1,)
    ...
    for i in range(N):
        ...
        Prior, TPPrior = RMatrix.PTBayes(Res, MP_Guess)
        runMaster = Levels.RunMaster(Res.E, MP_Guess.EB, Prior, TPPrior, MP_Guess.FreqAll, err=1e-15)
        ...
        Prior2, TPPrior2 = RMatrix.PTBayes(Res2, MP_Guess)
        runMaster2 = Levels.RunMaster(Res2.E, MP_Guess.EB, Prior2, TPPrior2, MP_Guess.FreqAll, err=1e-15)
