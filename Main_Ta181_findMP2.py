...
if __name__ == "__main__":
    ...
    mult = 3.5 # Missing resonance candidate density

    def run_ltp_loop(ratio, Res, Gnm, FreqTot, FreqF):
        # Gnm = params[2:]
        Freq_ = [ratio*FreqTot, (1-ratio)*FreqTot]
        # FreqF = pF * np.sum(Freq) / (1-pF)

        print('=================================')
        print(f'Freq1, Freq2, Gnm1, Gnm2, FreqF')
        print(f'{Freq_[0]:.8f},{Freq_[1]:.8f},{Gnm[0]:.8f},{Gnm[1]:.8f},{FreqF:.8f}')
        print('=================================')

        MP = Resonances.MeanParameters(Freq=Freq_, Gnm=Gnm, nDOF=dfn, Ggm=Ggm, gDOF=dfg, A=A, sg=SGs, EB=EB, FreqF=FreqF)
        
        Prior, TPPrior = PTBayes(Res, MP)
        def frac(e, g):
            return ((0.0 + 0.0000022*e**2)/29) * Freq_[g]/np.sum(Freq_)
    ...
    # Test 1: pF = 0, no missing
    # Test 2: pF = 0.05, no missing
    # Test 3: pF = 0.01, yes missing (mult = 2)

    K = 20
    Ratios = np.linspace(0.1, 0.9, K)
    FreqTot = np.sum([1/8.9868, 1/8.3065])
    Gnm  = [44.11355, 33.38697]
    FreqF = pF * FreqTot / (1-pF)
    MPE = np.zeros((K,2))
    ...
    for j, ratio in enumerate(Ratios):
        LTP[j] = run_ltp_loop(ratio, Res, Gnm, FreqTot, FreqF)
    Likihood = 10**(LTP-np.min(LTP))
    Likihood /= np.sum(Likihood)
    Mean = np.sum(Likihood * Ratios)
    STD  = np.sqrt(np.sum(Likihood*(Ratios-Mean)**2))
    print()
    # print(f'Truth    = {ratioT}')
    print(f'Estimate = {Mean:.5f} \u00B1 {STD:.5f}')
    print()
    print(Likihood)



    # for i, ratioT in enumerate(Ratios):
    #     Freq = FreqTot * np.array([ratioT, 1-ratioT])
    ...
    # # @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    # # Plotting the Estimation Error:
    # plt.figure()
    # plt.errorbar(Ratios, MPE[:,0], MPE[:,1], capsize=2, marker='.', ls='none', color='b')
    # plt.plot([0,1], [0,1], ':r')
    # plt.xlabel('True Mean Level Spacing Ratio', fontsize=16)
    # plt.ylabel('Estimate Mean Level Spacing Ratio', fontsize=16)
    # plt.tight_layout()
    # plt.show()
