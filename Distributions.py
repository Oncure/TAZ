...
def _high_order_level_spacing_parts(X, n, parts:tuple=(0,1,2)):
    """
    ...
    ...
    if n <= 15: # Lower n --> Exact Calculation
        ...
        rBX  = rB * X
        for part in parts:
            if   part == 0:     out.append(C * rBX**a * np.exp(-rBX**2))
            elif part == 1:     out.append((C/2) * gammaincc((a+1)/2, rBX**2))
            elif part == 2:     out.append((C/2) * gammaincc((a+2)/2, rBX**2))
    else: # Higher n --> Gaussian Approximation:
        sig = np.sqrt(_high_order_variance(n))
        for part in parts:
            if   part == 0:
                ...
                if 0 not in parts:      F0 = norm.pdf(X, n+1, sig)
                if 1 not in parts:      F1 = (1/2) * erfc((X+n+1)/(sig*math.sqrt(2)))
                out.append(sig**2*F0 + (X+1)*F1 - 1)
    return tuple(out)


class Distribution:
    f"""
    {__name__} is a class for level-spacing distributions, their integrals and inverses.
    """
    def __init__(self, **kwargs):
        """
        ...
        """
        if 'f0' in kwargs:      self.f0 = kwargs['f0']
        if 'f1' in kwargs:      self.f1 = kwargs['f1']
        if 'f2' in kwargs:      self.f2 = kwargs['f2']

        if 'parts' in kwargs:
            self.parts = kwargs['parts']
        elif 'f0' in kwargs and 'f1' in kwargs and 'f2' in kwargs:
            def parts(*X):
                F0 = kwargs['f0'](*X)
                F1 = kwargs['f1'](*X)
                F2 = kwargs['f2'](*X)
        ...
        if 'if1' in kwargs:      self.if1 = kwargs['if1']
        if 'if2' in kwargs:      self.if2 = kwargs['if2']

    @property
    def pdf(self):
        return self.f0

    def sample(self, size=None, Boundary:bool=False):
        """
        Inverse CDF Sampling
        """
        if Boundary:    return self.if2(np.rand(size))
        else:           return self.if1(np.rand(size))
    ...
    def wigner(cls, Freq:float=1.0):
        ...
        return cls(f0=get_f0, f1=get_f1, f2=get_f2, parts=get_parts, if1=get_if1, if2=get_if2)
    ...
    def missing(cls, Freq:float=1.0, pM:float=0.0, err:float=5e-3):
        ...
        """
        N_max = math.ceil(math.log(err, pM))
        coef = pM**np.arange(N_max+1)
        mult_fact = Freq * (1-pM) / (1 - pM**(N_max+1))
        def get_f0(X):
            func = lambda _n: _high_order_level_spacing_parts(X, _n, parts=(0,))
            Values = [func(n) for n in range(N_max+1)]
            return np.sum(coef * np.array([v[0] for v in Values]))
        def get_f1(X):
            func = lambda _n: _high_order_level_spacing_parts(X, _n, parts=(1,))
            Values = [func(n) for n in range(N_max+1)]
            return np.sum(coef * np.array([v[0] for v in Values]))
        def get_f2(X):
            func = lambda _n: _high_order_level_spacing_parts(X, _n, parts=(2,))
            Values = [func(n) for n in range(N_max+1)]
            return np.sum(coef * np.array([v[0] for v in Values]))
        def get_parts(X):
            func = lambda _n: _high_order_level_spacing_parts(X, _n, parts=(0,1,2))
            # Values = Parallel(n_jobs=-1, backend="threading")(map(delayed(func), range(N_max+1)))
            Values = [func(n) for n in range(N_max+1)]
            V0, V1, V2 = zip(*Values)
            F0 = np.sum(coef*V0)
            F1 = np.sum(coef*V1)
            F2 = np.sum(coef*V2)
            # F0 = np.sum(coef * np.array([v[0] for v in Values]))
            # F1 = np.sum(coef * np.array([v[1] for v in Values]))
            # F2 = np.sum(coef * np.array([v[2] for v in Values]))
            R1 = F0 / F1
            R2 = F1 / F2
            return F2, R1, R2
        def get_if1(X):
...
class Distributions:
    ...
    def parts(self, X):
        return np.array([distr.parts(X) for distr in self.distributions]).T

    def __getitem__(self, key):
        if hasattr(key, '__iter__'):
            return self.__class__(*self.distributions[key])
        else:
    ...
    def wigner(cls, *args, **kwargs):
        ...
        distr = []
        for j in range(N):
            argsj   = [arg[j] for arg in args]
            kwargsj = {kwarg : val[j] for kwarg, val in kwargs.items()}
            distr.append(Distribution.wigner(*argsj, **kwargsj))
        return cls(*distr)
    @classmethod
    def brody(cls, *args, **kwargs):
        if len(args) > 0:
            N = len(args[0])
        elif len(kwargs) > 0:
            N = len(kwargs.keys()[0])
        ...
        for j in range(N):
            argsj   = [arg[j] for arg in args]
            kwargsj = {kwarg : val[j] for kwarg, val in kwargs.items()}
            distr.append(Distribution.brody(*argsj, **kwargsj))
        return cls(*distr)
    @classmethod
    def missing(cls, *args, **kwargs):
        ...
        for j in range(N):
            argsj   = [arg[j] for arg in args]
            kwargsj = {kwarg : val[j] for kwarg, val in kwargs.items()}
            distr.append(Distribution.missing(*argsj, **kwargsj))
        return cls(*distr)